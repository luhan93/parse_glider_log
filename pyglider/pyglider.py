...
def load_glider_logs(indir):
    """ read all logs for glider """
    fns = glob.glob(indir+'/'+'*.log')
...
def parse_log_loc_vel(lines, glider):
    # msg_split_patt = r'From root\@dockserver'
    msg_split_patt = r'Connection Event:'
    msgs = re.split(msg_split_patt, ''.join(lines))
    ...
    for msg in gms:
        ...
        m = re.search(r'(sensor:m_final_water_vx.*?=)\s*((-?\d+?(\.+\d*)?)?(?:[eE][+\-]?\d+)?).*',msg,re.MULTILINE)
        if m and lat:
            v_str = m.group(0)
            final_vx = float(m.group(2))
            m = re.search(r'(\d*)\.\d* secs ago', v_str)
            if m:
                final_vx_secs_ago = int(m.group(1))
                final_vx_dt = dt - datetime.timedelta(0, final_vx_secs_ago, 0)
                # gps_dt_str = datetime.date.strftime(gps_dt, "%Y-%m-%d %H:%M:%S UTC")
            else:
                final_vx_dt = None
        else:
        ...
        m = re.search(r'(sensor:m_final_water_vy.*?=)\s*((-?\d+?(\.+\d*)?)?(?:[eE][+\-]?\d+)?).*', msg, re.MULTILINE)
        if m and lat:
            v_str = m.group(0)
            final_vy = float(m.group(2))
            m = re.search(r'(\d*)\.\d* secs ago', v_str)
            if m:
                final_vy_secs_ago = int(m.group(1))
                final_vy_dt = dt - datetime.timedelta(0, final_vy_secs_ago, 0)
                # gps_dt_str = datetime.date.strftime(gps_dt, "%Y-%m-%d %H:%M:%S UTC")
            else:
                final_vy_dt = None
        else:
        ...
        m = re.search(r'(sensor:m_water_vx.*?=)\s*((-?\d+?(\.+\d*)?)?(?:[eE][+\-]?\d+)?).*', msg, re.MULTILINE)
        if m and lat:
            v_str = m.group(0)
            vx = float(m.group(2))
            m = re.search(r'(\d*)\.\d* secs ago', v_str)
            if m:
                vx_secs_ago = int(m.group(1))
                vx_dt = dt - datetime.timedelta(0, vx_secs_ago, 0)
                # gps_dt_str = datetime.date.strftime(gps_dt, "%Y-%m-%d %H:%M:%S UTC")
            else:
                vx_dt = None
        else:
        ...
        m = re.search(r'(sensor:m_water_vy.*?=)\s*((-?\d+?(\.+\d*)?)?(?:[eE][+\-]?\d+)?).*', msg, re.MULTILINE)
        if m and lat:
            v_str = m.group(0)
            vy = float(m.group(2))
            m = re.search(r'(\d*)\.\d* secs ago', v_str)
            if m:
                vy_secs_ago = int(m.group(1))
                vy_dt = dt - datetime.timedelta(0, vy_secs_ago, 0)
                # gps_dt_str = datetime.date.strftime(gps_dt, "%Y-%m-%d %H:%M:%S UTC")
            else:
                vy_dt = None
        else:
        ...
        data.append({'glider': glider,
                         ...
                         'lat': lat,
                         'lon': lon,
                         'final_water_vx': final_vx,
                         'final_water_vx_dt': final_vx_dt,
                         'final_water_vy': final_vy,
                         'final_water_vy_dt': final_vy_dt,
                         'vx': vx,
                         'vx_dt': vx_dt,
                         'vy': vy,
                         'vy_dt': vy_dt
